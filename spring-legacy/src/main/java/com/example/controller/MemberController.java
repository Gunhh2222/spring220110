...
public class MemberController {
	...
	public ResponseEntity<String> login(String id, String passwd, boolean rememberMe, HttpServletResponse response,
		...
		// 4. 로그인 유지 체크했는지
		if (rememberMe == true) { // 로그인 유지 체크 했을 때
			// 쿠키등록하기
			Cookie cookie = new Cookie("userId", id);
			cookie.setMaxAge(60 * 60 * 24 * 7); // 쿠키 수명설정 초단위(60초*60분*24시*7일)
	...
	public ResponseEntity<String> passwd(String passwd,
			String newPasswd, String newPasswdConfirm,
			HttpSession session) {
		...
		if (newPasswd.equals(newPasswdConfirm) == false) { // 새밀번호, 새비밀번호 확인이 서로 다름
			HttpHeaders headers = new HttpHeaders();
			headers.add("Content-Type", "text/html; charset=UTF-8");

			String str = JScript.back("새 비밀번호가 일치하지 않습니다.");
